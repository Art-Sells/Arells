...
const ADA_VAPA_KEY = 'vavity/cardanoVAPA.json';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const data = await s3.getObject({ Bucket: s3BucketNameOrThrow(), Key: ADA_VAPA_KEY }).promise();
    ...
    console.error('[cardanoVapaHistoricalPrice] error', error);
