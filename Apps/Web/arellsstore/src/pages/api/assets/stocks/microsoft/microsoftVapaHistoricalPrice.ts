...
const VAPA_KEY = STOCK_ASSET_BY_ID.microsoft.s3VapaKey;
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const data = await s3.getObject({ Bucket: s3BucketNameOrThrow(), Key: VAPA_KEY }).promise();
    ...
    console.error('[microsoftVapaHistoricalPrice] error', error);
