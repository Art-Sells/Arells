...
export async function sendVerificationEmail(opts: {
  ...
  if (!from) {
    console.warn('[auth] EMAIL_FROM not set; cannot send verification email. URL:', opts.verifyUrl);
  ...
  const subject = 'Arells Verification';
  ...
  <div style="max-width:480px;margin:0 auto;background:#fff;border-radius:12px;padding:24px;border:1px solid #e0e0e0;">
    ...
    <h1 style="font-size:18px;margin:0 0 12px;">Arells Verification</h1>
