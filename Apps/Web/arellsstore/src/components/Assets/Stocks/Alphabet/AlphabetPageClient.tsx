...
/** Opaque viewport tint for overscroll (Alphabet blue wash, not plain white). */
const ALPHABET_PAGE_OVERSCROLL_BG = 'rgb(232, 240, 254)';
...
const AlphabetPageClient: React.FC = () => {
  ...
  /** Survives `<Alphabet key={sessionResetKey} />` remounts so session-clear-on-mount runs once per page visit. */
  ...
  useEffect(() => {
    ...
    const bg = ALPHABET_PAGE_OVERSCROLL_BG;
  ...
  return (
    <div className={`asset-page asset-page--${ASSET.cssModifier}`} ref={pageRef}>
      ...
      <Alphabet key={`session-reset-${sessionResetKeyValue}`} sessionMountClearGuardRef={sessionMountClearGuardRef} />
...
export default AlphabetPageClient;
