...
const BNB_VAPA_KEY = 'vavity/bnbVAPA.json';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const data = await s3.getObject({ Bucket: s3BucketNameOrThrow(), Key: BNB_VAPA_KEY }).promise();
    ...
    console.error('[bnbVapaHistoricalPrice] error', error);
