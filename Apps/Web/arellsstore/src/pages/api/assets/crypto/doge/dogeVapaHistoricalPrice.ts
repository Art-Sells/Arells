...
const DOGE_VAPA_KEY = 'vavity/dogeVAPA.json';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const data = await s3.getObject({ Bucket: s3BucketNameOrThrow(), Key: DOGE_VAPA_KEY }).promise();
    ...
    console.error('[dogeVapaHistoricalPrice] error', error);
