...
// Track when the module was first loaded
const moduleStartTime = Date.now();

// Calculate the current synthetic price based on the pattern
export const getSyntheticPrice = (): number => {
  const now = Date.now();
  // Calculate seconds since module was loaded, not since Unix epoch!
  const secondsSinceStart = Math.floor((now - moduleStartTime) / MILLISECONDS_PER_SECOND);
