...
const XRP_VAPA_KEY = 'vavity/xrpVAPA.json';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const data = await s3.getObject({ Bucket: s3BucketNameOrThrow(), Key: XRP_VAPA_KEY }).promise();
    ...
    console.error('[xrpVapaHistoricalPrice] error', error);
