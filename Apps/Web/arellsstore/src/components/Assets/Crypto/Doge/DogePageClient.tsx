...
/** Opaque viewport tint for overscroll (DOGE gold wash, not plain white). */
const DOGE_PAGE_OVERSCROLL_BG = 'rgb(255, 248, 232)';
...
const DogePageClient: React.FC = () => {
  ...
  /** Survives `<Doge key={sessionResetKey} />` remounts so session-clear-on-mount runs once per page visit. */
  ...
  useEffect(() => {
    ...
    const bg = DOGE_PAGE_OVERSCROLL_BG;
  ...
  return (
    <div className={`asset-page asset-page--${ASSET.cssModifier}`} ref={pageRef}>
            ...
            <div className={`asset-session-reset-spinner-wrap asset-profit-summary asset-profit-summary--${ASSET.cssModifier}`}>
              <div className="asset-session-reset-spinner" aria-hidden="true">
                <div
                  ...
                  style={{ borderColor: 'rgba(194, 166, 51, 0.2)', borderTopColor: 'rgba(194, 166, 51, 0.5)' }}
                />
              </div>
            </div>
      ...
      <Doge key={`session-reset-${sessionResetKeyValue}`} sessionMountClearGuardRef={sessionMountClearGuardRef} />
