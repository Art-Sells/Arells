...
const DogeChart: React.FC<Props> = ({
  history = [],
  color = 'rgba(194, 166, 51, 0.9)',
  activeColor = 'rgba(194, 166, 51, 1)',
  markerColor = 'rgba(194, 166, 51, 1)',
  gridColor = 'rgba(194, 166, 51, 0.2)',
  gridSpacing = 20,
  height = 300,
  interactiveHeight,
  canvasOffsetTop = 0,
  ...
  const solveTForX = useCallback(
    (xTarget: number, x0: number, x1: number, x2: number, x3: number) => {
      // Bezier x(t) should be monotonic for time series; use binary search for robustness.
      let lo = 0;
      let hi = 1;
  ...
  const resolvePointAtPixel = useCallback(
    (pixelX: number | null) => {
      ...
      // Find the segment [i, i+1] such that x is within it.
      let segIdx = 0;
      ...
      // Fallback: linear interpolation in value space
