...
const TRON_VAPA_KEY = 'vavity/tronVAPA.json';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const data = await s3.getObject({ Bucket: s3BucketNameOrThrow(), Key: TRON_VAPA_KEY }).promise();
    ...
    console.error('[tronVapaHistoricalPrice] error', error);
