...
/** Opaque viewport tint for overscroll (Microsoft wash, not plain white). */
const MICROSOFT_PAGE_OVERSCROLL_BG = 'rgb(232, 242, 252)';
...
const MicrosoftPageClient: React.FC = () => {
  ...
  /** Survives `<Microsoft key={sessionResetKey} />` remounts so session-clear-on-mount runs once per page visit. */
  ...
  useEffect(() => {
    ...
    const bg = MICROSOFT_PAGE_OVERSCROLL_BG;
  ...
  useEffect(() => {
    ...
    document
      .querySelectorAll<HTMLLinkElement>(
        'link[rel="shortcut icon"], link[rel="icon"][type="image/svg+xml"], link[rel="microsoft-touch-icon"][type="image/svg+xml"]'
      )
  ...
  return (
    <div className={`asset-page asset-page--${ASSET.cssModifier}`} ref={pageRef}>
            ...
            <div className={`asset-session-reset-spinner-wrap asset-profit-summary asset-profit-summary--${ASSET.cssModifier}`}>
              <div className="asset-session-reset-spinner" aria-hidden="true">
                <div
                  ...
                  style={{ borderColor: 'rgba(0, 120, 212, 0.2)', borderTopColor: 'rgba(0, 120, 212, 0.5)' }}
                />
              </div>
            </div>
      ...
      <Microsoft key={`session-reset-${sessionResetKeyValue}`} sessionMountClearGuardRef={sessionMountClearGuardRef} />
...
export default MicrosoftPageClient;
