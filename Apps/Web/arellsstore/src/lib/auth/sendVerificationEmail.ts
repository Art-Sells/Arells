...
export async function sendVerificationEmail(opts: {
  ...
  if (process.env.AUTH_EMAIL_DISABLED === '1') {
    console.info('[auth] AUTH_EMAIL_DISABLED: verification code for', opts.to, opts.code);
    return { sent: false, skippedReason: 'AUTH_EMAIL_DISABLED' };
  }

  const fromAddr = process.env.VERIFY_EMAIL_FROM?.trim();
  if (!fromAddr) {
    console.warn('[auth] VERIFY_EMAIL_FROM not set; cannot send verification email. Code:', opts.code);
