...
const BCH_VAPA_KEY = 'vavity/bchVAPA.json';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const data = await s3.getObject({ Bucket: s3BucketNameOrThrow(), Key: BCH_VAPA_KEY }).promise();
    ...
    console.error('[bchVapaHistoricalPrice] error', error);
