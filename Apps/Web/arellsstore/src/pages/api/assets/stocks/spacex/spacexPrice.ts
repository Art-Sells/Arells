...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (cachedPrice != null && cacheTimestamp && currentTime - cacheTimestamp < CACHE_DURATION) {
    res.status(200).json({ spacex: { usd: cachedPrice } });
    return;
  }
  ...
  try {
    ...
    res.status(200).json({ spacex: { usd: price } });
  } catch (error) {
    console.error('Error fetching SpaceX price:', error);
    if (cachedPrice != null) {
      res.status(200).json({ spacex: { usd: cachedPrice } });
      return;
    }
    res.status(500).json({ error: 'Error fetching SpaceX price' });
  }
}
