...
/** Opaque viewport tint for overscroll (SpaceX blue wash, not plain white). */
const SPCX_PAGE_OVERSCROLL_BG = 'rgb(244, 245, 247)';
...
const SpaceXPageClient: React.FC = () => {
  ...
  /** Survives `<SpaceX key={sessionResetKey} />` remounts so session-clear-on-mount runs once per page visit. */
  ...
  useEffect(() => {
    ...
    const bg = SPCX_PAGE_OVERSCROLL_BG;
  ...
  return (
    <div className={`asset-page asset-page--${ASSET.cssModifier}`} ref={pageRef}>
      ...
      <SpaceX key={`session-reset-${sessionResetKeyValue}`} sessionMountClearGuardRef={sessionMountClearGuardRef} />
...
export default SpaceXPageClient;
