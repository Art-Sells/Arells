...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (cachedPrice != null && cacheTimestamp && currentTime - cacheTimestamp < CACHE_DURATION) {
    res.status(200).json({ microsoft: { usd: cachedPrice } });
    return;
  }
  ...
  try {
    ...
    res.status(200).json({ microsoft: { usd: price } });
  } catch (error) {
    console.error('Error fetching MSFT price:', error);
    if (cachedPrice != null) {
      res.status(200).json({ microsoft: { usd: cachedPrice } });
      return;
    }
    res.status(500).json({ error: 'Error fetching MSFT price' });
  }
}
