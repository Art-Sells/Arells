...
const BnbPageClient: React.FC = () => {
  ...
  /** Survives `<Bnb key={sessionResetKey} />` remounts so session-clear-on-mount runs once per page visit. */
  ...
  useEffect(() => {
    ...
    const bg = BNB_PAGE_OVERSCROLL_BG;
  ...
  useEffect(() => {
    const badgeUrl = iconAssetUrl('/images/favicons/BnbBadge.svg');
    document
  ...
  return (
    <div className={`asset-page asset-page--${ASSET.cssModifier}`} ref={pageRef}>
            ...
            <div className={`asset-session-reset-spinner-wrap asset-profit-summary asset-profit-summary--${ASSET.cssModifier}`}>
              <div className="asset-session-reset-spinner" aria-hidden="true">
                <div
                  ...
                  style={{ borderColor: 'rgba(243, 186, 47, 0.2)', borderTopColor: 'rgba(243, 186, 47, 0.5)' }}
                />
              </div>
            </div>
      ...
      <Bnb key={`session-reset-${sessionResetKeyValue}`} sessionMountClearGuardRef={sessionMountClearGuardRef} />
