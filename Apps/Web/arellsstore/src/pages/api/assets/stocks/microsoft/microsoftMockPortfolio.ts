  ...
  try {
    ...
    try {
      const resp = await axios
        .get('http://localhost:3000/api/assets/stocks/microsoft/microsoftvapa')
        .catch(() => axios.get('/api/assets/stocks/microsoft/microsoftvapa'));
      vapa = typeof resp.data?.vapa === 'number' ? resp.data.vapa : 0;
      history = Array.isArray(resp.data?.solidHistory)
        ? resp.data.solidHistory
    ...
    return res.status(500).json({ error: 'Failed to generate mock portfolio', details: err?.message });
  }
}
