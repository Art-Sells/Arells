...
/** Opaque viewport tint for overscroll (Chainlink blue wash, not plain white). */
const LINK_PAGE_OVERSCROLL_BG = 'rgb(246, 248, 255)';
...
const ChainlinkPageClient: React.FC = () => {
  ...
  /** Survives `<Chainlink key={sessionResetKey} />` remounts so session-clear-on-mount runs once per page visit. */
  ...
  useEffect(() => {
    ...
    const bg = LINK_PAGE_OVERSCROLL_BG;
  ...
  return (
    <div className={`asset-page asset-page--${ASSET.cssModifier}`} ref={pageRef}>
      ...
      <Chainlink key={`session-reset-${sessionResetKeyValue}`} sessionMountClearGuardRef={sessionMountClearGuardRef} />
...
export default ChainlinkPageClient;
