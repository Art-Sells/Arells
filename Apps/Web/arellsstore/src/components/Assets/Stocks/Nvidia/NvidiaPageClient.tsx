...
/** Opaque viewport tint for overscroll (Nvidia blue wash, not plain white). */
const NVDA_PAGE_OVERSCROLL_BG = 'rgb(242, 246, 232)';
...
const NvidiaPageClient: React.FC = () => {
  ...
  /** Survives `<Nvidia key={sessionResetKey} />` remounts so session-clear-on-mount runs once per page visit. */
  ...
  useEffect(() => {
    ...
    const bg = NVDA_PAGE_OVERSCROLL_BG;
  ...
  return (
    <div className={`asset-page asset-page--${ASSET.cssModifier}`} ref={pageRef}>
      ...
      <Nvidia key={`session-reset-${sessionResetKeyValue}`} sessionMountClearGuardRef={sessionMountClearGuardRef} />
...
export default NvidiaPageClient;
