...
const SOL_VAPA_KEY = 'vavity/solanaVAPA.json';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const data = await s3.getObject({ Bucket: s3BucketNameOrThrow(), Key: SOL_VAPA_KEY }).promise();
    ...
    console.error('[solanaVapaHistoricalPrice] error', error);
