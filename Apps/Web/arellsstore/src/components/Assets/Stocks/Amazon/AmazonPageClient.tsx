...
/** Opaque viewport tint for overscroll (Amazon wash, not plain white). */
const AMAZON_PAGE_OVERSCROLL_BG = 'rgb(255, 248, 240)';
...
const AmazonPageClient: React.FC = () => {
  ...
  /** Survives `<Amazon key={sessionResetKey} />` remounts so session-clear-on-mount runs once per page visit. */
  ...
  useEffect(() => {
    ...
    const bg = AMAZON_PAGE_OVERSCROLL_BG;
  ...
  useEffect(() => {
    ...
    document
      .querySelectorAll<HTMLLinkElement>(
        'link[rel="shortcut icon"], link[rel="icon"][type="image/svg+xml"], link[rel="amazon-touch-icon"][type="image/svg+xml"]'
      )
  ...
  return (
    <div className={`asset-page asset-page--${ASSET.cssModifier}`} ref={pageRef}>
            ...
            <div className={`asset-session-reset-spinner-wrap asset-profit-summary asset-profit-summary--${ASSET.cssModifier}`}>
              <div className="asset-session-reset-spinner" aria-hidden="true">
                <div
                  ...
                  style={{ borderColor: 'rgba(196, 110, 0, 0.2)', borderTopColor: 'rgba(196, 110, 0, 0.5)' }}
                />
              </div>
            </div>
      ...
      <Amazon key={`session-reset-${sessionResetKeyValue}`} sessionMountClearGuardRef={sessionMountClearGuardRef} />
...
export default AmazonPageClient;
