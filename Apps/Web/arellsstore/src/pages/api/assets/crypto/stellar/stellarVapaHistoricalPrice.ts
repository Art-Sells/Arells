...
const STELLAR_VAPA_KEY = 'vavity/stellarVAPA.json';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const data = await s3.getObject({ Bucket: s3BucketNameOrThrow(), Key: STELLAR_VAPA_KEY }).promise();
    ...
    console.error('[stellarVapaHistoricalPrice] error', error);
