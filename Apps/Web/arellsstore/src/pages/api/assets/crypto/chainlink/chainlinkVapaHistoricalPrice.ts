...
const LINK_VAPA_KEY = 'vavity/chainlinkVAPA.json';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const data = await s3.getObject({ Bucket: s3BucketNameOrThrow(), Key: LINK_VAPA_KEY }).promise();
    ...
    console.error('[chainlinkVapaHistoricalPrice] error', error);
