...
/** Opaque viewport tint for overscroll (Cardano blue wash, not plain white). */
const ADA_PAGE_OVERSCROLL_BG = 'rgb(244, 246, 255)';
...
const CardanoPageClient: React.FC = () => {
  ...
  /** Survives `<Cardano key={sessionResetKey} />` remounts so session-clear-on-mount runs once per page visit. */
  ...
  useEffect(() => {
    ...
    const bg = ADA_PAGE_OVERSCROLL_BG;
  ...
  return (
    <div className={`asset-page asset-page--${ASSET.cssModifier}`} ref={pageRef}>
            ...
            <div className={`asset-session-reset-spinner-wrap asset-profit-summary asset-profit-summary--${ASSET.cssModifier}`}>
              <div className="asset-session-reset-spinner" aria-hidden="true">
                <div
                  ...
                  style={{ borderColor: 'rgba(0, 51, 173, 0.2)', borderTopColor: 'rgba(0, 51, 173, 0.5)' }}
                />
              </div>
            </div>
      ...
      <Cardano key={`session-reset-${sessionResetKeyValue}`} sessionMountClearGuardRef={sessionMountClearGuardRef} />
...
export default CardanoPageClient;
