...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (cachedPrice != null && cacheTimestamp && currentTime - cacheTimestamp < CACHE_DURATION) {
    res.status(200).json({ amazon: { usd: cachedPrice } });
    return;
  }
  ...
  try {
    ...
    res.status(200).json({ amazon: { usd: price } });
  } catch (error) {
    console.error('Error fetching AMZN price:', error);
    if (cachedPrice != null) {
      res.status(200).json({ amazon: { usd: cachedPrice } });
      return;
    }
    res.status(500).json({ error: 'Error fetching AMZN price' });
  }
}
