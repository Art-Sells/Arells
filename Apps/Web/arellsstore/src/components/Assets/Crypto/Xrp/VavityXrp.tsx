...
const VavityXrp: React.FC<VavityXrpProps> = ({ sessionMountClearGuardRef }) => {
  ...
  const assetSnapshot = getAsset('xrp');
    ...
    const loadData = async () => {
      ...
      try {
        const data = isSignedIn
          ? await (async () => {
              const params = new URLSearchParams({ email, asset: 'xrp' });
              const res = await fetch(`/api/user/fetchUserVavityAggregator?${params.toString()}`);
              return await res.json();
            })()
          : await fetchVavityAggregator(sessionId, 'xrp');
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!hasInvestments) {
          const current = await fetchVavityAggregator(sessionId, 'xrp');
        ...
        await saveVavityAggregator(sessionId, [], 'xrp');
        const cleared = await fetchVavityAggregator(sessionId, 'xrp');
  ...
  const handleSubmitInvestment = async () => {
    ...
    try {
      ...
      if (isSignedIn) {
        await addEmailInvestments('xrp', [newInvestment]);
        const params = new URLSearchParams({ email, asset: 'xrp' });
        const res = await fetch(`/api/user/fetchUserVavityAggregator?${params.toString()}`);
        refreshed = await res.json();
      } else {
        await addVavityAggregator(sessionId, [newInvestment], 'xrp');
        refreshed = await fetchVavityAggregator(sessionId, 'xrp');
      }
  ...
  const handleDeleteInvestment = async (investmentId: string) => {
    ...
    try {
      ...
      if (isSignedIn) {
        await saveEmailInvestmentsForAsset('xrp', updated);
        const params = new URLSearchParams({ email, asset: 'xrp' });
        const res = await fetch(`/api/user/fetchUserVavityAggregator?${params.toString()}`);
        ...
        await saveVavityAggregator(sessionId, updated, 'xrp');
        const refreshed = await fetchVavityAggregator(sessionId, 'xrp');
        if (debugDelete && lastDeletedSignatureRef.current && Array.isArray(refreshed?.investments)) {
          const signature = lastDeletedSignatureRef.current;
          const returned = refreshed.investments.some((entry: any) => {
            const sig =
          ...
          <div className="asset-invest-form-body asset-invest-form-body--xrp">
            ...
            <div className="asset-invest-form-controls asset-invest-form-controls--xrp">
              ...
              <div className="asset-invest-form-field">
                ...
                <div className="asset-invest-form-field-control">
                  <CustomDatePicker value={purchaseDate} onChange={setPurchaseDate} placeholder="MM/DD/YYYY" asset="xrp" />
                </div>
              </div>
