...
/** Opaque viewport tint for overscroll (Bitcoin Cash green wash, not plain white). */
const BCH_PAGE_OVERSCROLL_BG = 'rgb(244, 255, 251)';
...
const BitcoinCashPageClient: React.FC = () => {
  ...
  /** Survives `<BitcoinCash key={sessionResetKey} />` remounts so session-clear-on-mount runs once per page visit. */
  ...
  useEffect(() => {
    ...
    const bg = BCH_PAGE_OVERSCROLL_BG;
  ...
  return (
    <div className={`asset-page asset-page--${ASSET.cssModifier}`} ref={pageRef}>
            ...
            <div className={`asset-session-reset-spinner-wrap asset-profit-summary asset-profit-summary--${ASSET.cssModifier}`}>
              <div className="asset-session-reset-spinner" aria-hidden="true">
                <div
                  ...
                  style={{ borderColor: 'rgba(10, 193, 142, 0.2)', borderTopColor: 'rgba(10, 193, 142, 0.5)' }}
                />
              </div>
            </div>
      ...
      <BitcoinCash key={`session-reset-${sessionResetKeyValue}`} sessionMountClearGuardRef={sessionMountClearGuardRef} />
...
export default BitcoinCashPageClient;
